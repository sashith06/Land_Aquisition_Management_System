              ...
              {valuation.insights && (
                <div className="bg-white border-2 border-purple-300 rounded-xl p-6 shadow-xl">
                  ...
                  <div className="flex items-center gap-3 mb-5 pb-4 border-b-2 border-purple-200">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-gray-800">AI-Powered Market Insights</h3>
                      <p className="text-sm text-purple-600 font-medium">Generated by Google Gemini AI</p>
                    </div>
                  </div>

                  {/* Insights Content */}
                  <div className="space-y-5">
                    {valuation.insights.split('\n\n').map((paragraph, index) => {
                      // Remove markdown bold/italic markers
                      const cleanParagraph = paragraph.replace(/\*\*/g, '').replace(/\*/g, '');
                      
                      // Check if it's a heading (starts with # or contains certain keywords)
                      if (cleanParagraph.trim().startsWith('#')) {
                        const heading = cleanParagraph.replace(/^#+\s*/, '').trim();
                        return (
                          <div key={index} className="mt-6 mb-3">
                            <h4 className="text-lg font-bold text-gray-800 flex items-center gap-3 bg-purple-50 px-4 py-3 rounded-lg border-l-4 border-purple-500">
                      ...
                      if (cleanParagraph.includes('•') || paragraph.includes('*') || cleanParagraph.match(/^[\s]*[-•]\s/m)) {
                          ...
                          <div key={index} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                            <ul className="space-y-3 list-disc list-inside">
                              {items.map((item, itemIndex) => {
                                ...
                                if (cleanItem) {
                                  return (
                                    <li key={itemIndex} className="text-gray-700 text-sm leading-relaxed font-medium pl-2">
                                      {cleanItem}
                                    </li>
                                  );
                                }
                      ...
                      // Check if it's a key-value pair or important note
                      if (cleanParagraph.includes(':') && cleanParagraph.split('\n').length === 1) {
                        const [key, ...valueParts] = cleanParagraph.split(':');
                        const value = valueParts.join(':').trim();
                        return (
                          <div key={index} className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg p-4 border-l-4 border-purple-500 shadow-sm">
                            <p className="text-sm">
                              <span className="font-bold text-purple-900 text-base">{key}:</span>
                              <span className="text-gray-700 ml-2 font-medium">{value}</span>
                            </p>
                          </div>
                        );
                      }
                  ...
                  <div className="mt-6 pt-4 border-t-2 border-gray-200">
                    <div className="flex items-start gap-3 bg-yellow-50 p-4 rounded-lg border border-yellow-200">
                      ...
                      <p className="text-xs text-gray-700 leading-relaxed">
                        <strong className="text-yellow-800 text-sm">Important Note:</strong> These insights are AI-generated based on current market data and analysis. 
                        They should be used as a supplementary tool alongside professional valuation advice.
                      </p>
                    </div>
                  </div>
                </div>
